...
cmd_conv = "ffmpeg -i #{input} -filter_complex \"[0]split[body][pre];[pre]trim=duration=#{fade},format=yuva420p,fade=d=#{fade}:alpha=1,setpts=PTS+(#{length-fade}/TB)[jt];[body]trim=0,setpts=PTS-STARTPTS[main];[main][jt]overlay\" #{output}"
system cmd_conv
